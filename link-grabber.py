...
class RoundedButton(tk.Canvas):
    ...
    def on_enter(self, e): self.config(cursor="hand2")
    def on_leave(self, e): self.config(cursor="")
...
class LinkGrabberApp:
    def __init__(self, root):
        ...
        # Set window icon (works for PyInstaller exe)
        try:
            self.root.iconbitmap(APP_ICON)
        except: pass

        # Make messageboxes use same icon
    ...
    def setup_ui(self):
        ...
        tk.Label(container, text="Link Grabber", font=("Segoe UI",22,"bold"), bg="#1e1e1e", fg="#ffffff").pack(pady=(0,3))
        tk.Label(container, text="Extract fuckingfast.co download links", font=("Segoe UI",9), bg="#1e1e1e", fg="#b0b0b0").pack(pady=(0,15))

        # Input
        input_frame = tk.Frame(container, bg="#2d2d2d", highlightthickness=1, highlightbackground="#3d3d3d")
        input_frame.pack(fill=tk.BOTH, expand=True, pady=(0,15))
        tk.Label(input_frame, text="Paste URLs Here", font=("Segoe UI",9,"bold"), bg="#2d2d2d", fg="#ffffff").pack(anchor=tk.W, padx=12, pady=(8,5))
        self.input_text = tk.Text(input_frame, font=("Consolas",9), bg="#252525", fg="#e0e0e0", relief=tk.FLAT, padx=12, pady=8, wrap=tk.WORD, insertbackground="#3498db", selectbackground="#3d5875", selectforeground="#ffffff")
        ...
        # Status
        self.status_label = tk.Label(container, text="Ready", font=("Segoe UI",8), bg="#1e1e1e", fg="#b0b0b0")
        ...
        # Log
        log_frame = tk.Frame(container, bg="#2d2d2d", highlightthickness=1, highlightbackground="#3d3d3d")
        log_frame.pack(fill=tk.BOTH, expand=True)
        tk.Label(log_frame, text="Activity Log", font=("Segoe UI",9,"bold"), bg="#2d2d2d", fg="#ffffff").pack(anchor=tk.W, padx=12, pady=(8,5))
        self.log_text = tk.Text(log_frame,font=("Consolas",8),bg="#252525",fg="#e0e0e0",relief=tk.FLAT,state=tk.DISABLED,padx=12,pady=8,wrap=tk.WORD,selectbackground="#3d5875",selectforeground="#ffffff")
    ...
    def update_progress_bar(self):
        ...
        w = c.winfo_width()
        if w<2: return
        self.create_rounded_rect(c,0,0,w,12,6,"#2d2d2d")
        pw = (self.progress_value/self.progress_max)*w
        if pw>0: self.create_rounded_rect(c,0,0,pw,12,6,"#3498db")

    def create_rounded_rect(self,c,x1,y1,x2,y2,r,color):
        points = [x1+r,y1,x1+r,y1,x2-r,y1,x2-r,y1,x2,y1,x2,y1+r,x2,y1+r,x2,y2-r,x2,y2-r,x2,y2,x2-r,y2,x2-r,y2,x1+r,y2,x1+r,y2,x1,y2,x1,y2-r,x1,y2-r,x1,y1+r,x1,y1+r,x1,y1]
        c.create_polygon(points, fill=color, outline="", smooth=True)
    ...
    def extract_download_link(self,url):
        try:
            ...
            m = re.search(r'window\.open\("(https://fuckingfast\.co/dl/[^"]+)"\)',r.text)
            return (m.group(1), None) if m else (None,"Link not found")
    ...
    def start_extraction(self):
        ...
        for line in urls_text.split("\n"):
            ...
            if "fuckingfast.co" in line: urls.append(line)
        if not urls:
    ...
    def extract_links(self,urls):
        ...
        self.stop_btn.configure_state("disabled")
        if self.extracted_links: self.save_btn.configure_state("normal")
        self.status_label.config(text=f"Done - {success} links extracted")
        if success>0: messagebox.showinfo("Success",f"Extracted {success} links!",parent=self.msgbox_root)
    ...
    def save_links(self):
        if not self.extracted_links:
            ...
            return
        filename=filedialog.asksaveasfilename(defaultextension=".txt",filetypes=[("Text files","*.txt"),("All files","*.*")],initialfile="download_links.txt")
        if filename:
            try:
                with open(filename,"w",encoding="utf-8") as f:
                    for link in self.extracted_links: f.write(link+"\n")
                messagebox.showinfo("Saved",f"Saved {len(self.extracted_links)} links!\n\nImport to IDM:\nTasks → Import → Import from text file",parent=self.msgbox_root)
